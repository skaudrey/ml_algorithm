...
def makeGif(figDir,figName,method=1,format =  '.png'):
    __imageio = 1
    __PIL = 2

    images = []

    filenames = sorted(fn for fn in os.listdir(figDir) if fn.endswith(format))
    if method==__imageio:
...
if __name__ =='__main__':
    figName = 'occur_freq_month.gif'
    format = '.jpg'

    imgDir = ''

    makeGif(imgDir,figName,format=format)